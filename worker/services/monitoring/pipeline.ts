...
export function rowToState(row: StateRow): ParticipantState {
  return {
    ...
    blinkRatePerMin: row.blinkRatePerMin,
...
function stateToRow(
  ...
  return {
    ...
    blinkRatePerMin: state.blinkRatePerMin,
...
export async function applyObservation(
  ...
  if (config.observationRetentionDays > 0) {
    await db.insert(participantObservations).values({
      ...
      blinkRatePerMin: observation.blinkRatePerMin ?? null,
      sharpness: observation.sharpness ?? null,
