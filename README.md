...
## 5.5 Zoom会議インテリジェンス層（主催者コンソール）

Zoom会議の**主催者**向けに、参加者の在席・本人確認・画面正対・イベント・レポートを
一元管理する層を追加しました。既存機能（受講者登録・研修管理・ライブ監視・証跡）は
すべてそのまま動作します。

```
Zoom会議 ─► Zoom統合アダプタ（Meeting SDK / RTMS / シミュレーション）
        ─► 参加者状態サービス ─► 解析スケジューラ（HOT/WARM/NORMAL）
        ─► Ayonix視覚パイプライン（GPUホスト上。Workers上では実行しません）
        ─► イベントエンジン（時間的持続・重複排除・開始/解消）
        ─► Durable Object（WebSocket）─► 主催者ダッシュボード
```

- 画面: **ライブ会議 / 参加者 / イベント / レポート**（既存6画面はそのまま）
- 観測できる事象のみを扱い、**理解度・集中度・心理状態は判定しません**
- 参加者の生映像をブラウザへ再送しません（サムネイルと解析結果のみ）
- 設定 → **会議モニタリング** でFPS・しきい値・保存期間を組織ごとに構成

Zoom接続なしで全画面を検証できる**シミュレーション**を同梱しています
（ライブ会議 → 解析を開始 → シミュレーションで開始 → データを生成）。

詳細は **`docs/MEETING_INTELLIGENCE.md`**（アーキテクチャ、API、互換性チェックリスト、
セキュリティレビュー、Zoomの制約、Video SDKへの移行方針）を参照してください。
